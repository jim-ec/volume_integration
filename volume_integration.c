...
void readPolyhedron(char *name, struct Polyhedron *p)
{
    FILE *fp;
    int i, j;
    ...
    for (i = 0; i < p->numFaces; i++)
    {
        ...
        f->norm[Y] = ny / len;
        f->norm[Z] = nz / len;
        f->w = -f->norm[X] * p->verts[f->verts[0]][X] - f->norm[Y] * p->verts[f->verts[0]][Y] - f->norm[Z] * p->verts[f->verts[0]][Z];

        printf("%lf %lf %lf %lf\n", f->norm[X], f->norm[Y], f->norm[Z], f->w);
    }
...
void compProjectionIntegrals(struct Face *f)
{
    double a0, a1, da;
    double b0, b1, db;
...
void compFaceIntegrals(struct Face *f)
{
    double *n, w;
    double k1, k2, k3, k4;
...
void compVolumeIntegrals(struct Polyhedron *p)
{
    struct Face *f;
    double nx, ny, nz;
    int i;
